//dont print files with more then N entries
...
pub fn print_file_n_limit(name: String, path:String, depth:u32,n:usize) -> Pretty{
  ...
  let directory = match read_dir(path.clone()){
   ...
   let directory_count = match read_dir(path.clone()){
      Ok(directory) => directory.count(),
      Err(_) =>  {
      let fail = Pretty {
                  found:res,
                  folders:0,
          ...
          Ok(place) => {
            ...
            // now print each folder and file
               ...
               if place.is_file() { 
                  ...
                  let res2 = print_file_n_limit(new_name,string_path, depth+1,n);
